...
def load_text_encoder(
    text_encoder_type, text_encoder_path, text_encoder_precision=None
):
    print(f"Loading text encoder model ({text_encoder_type}) from: {text_encoder_path}")
    ...
    if text_encoder_precision is not None:
        text_encoder = text_encoder.to(
            dtype=constants.PRECISION_TO_TYPE[text_encoder_precision]
        )
...
def load_tokenizer(tokenizer_type, tokenizer_path, padding_side="right"):
    print(f"Loading tokenizer ({tokenizer_type}) from: {tokenizer_path}")
...
class TextEncoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.context_input_key = context_input_key
        self.precision = text_encoder_precision
        self.model_path = text_encoder_path if text_encoder_path is not None else model
        self.tokenizer_type = (
            tokenizer_type if tokenizer_type is not None else text_encoder_type
        )
        self.tokenizer_path = tokenizer_path if tokenizer_path is not None else model
    ...
    def forward(
        ...
        **kwargs,
        ...
        text = context[self.context_input_key]
        batch_encoding = self.text2tokens(text)
        prompt_outputs = self.encode(
            batch_encoding,
