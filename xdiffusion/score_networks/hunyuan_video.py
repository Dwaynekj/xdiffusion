...
from xdiffusion.layers.flux import MLPEmbedder
from xdiffusion.layers.hunyuan_video.attention import (
    attention,
    parallel_attention,
    get_cu_seqlens,
)
from xdiffusion.layers.hunyuan_video.embedding import (
    TimestepEmbedder,
    PatchEmbed,
    TextProjection,
    FinalLayer,
...
class MMDoubleStreamBlock(nn.Module):
    ...
    def __init__(
        ...
        self.img_mod = ModulateDiT(
            hidden_size,
            factor=6,
            act_layer=get_activation("silu", return_cls=True),
        )
        ...
        self.img_mlp = Mlp(
            hidden_size,
            mlp_hidden_dim,
            act_layer=get_activation(mlp_act_type, return_cls=True),
            bias=True,
        )
...
class MMSingleStreamBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp_act = get_activation(mlp_act_type)
    ...
    def forward(
        ...
        # clip_states_dim: int = 768,
        # text_states_dim: int = 4096,


class HYVideoDiffusionTransformer(torch.nn.Module):
    """
    ...
    [3] Hunyuan Video: https://arxiv.org/abs/2412.03603
    ...
    def __init__(
        ...
        self.clip_states_dim = config.clip_states_dim

        if config.hidden_size % config.heads_num != 0:
            raise ValueError(
                f"Hidden size {config.hidden_size} must be divisible by heads_num {config.heads_num}"
            )
        pe_dim = config.hidden_size // config.heads_num
        if sum(config.rope_dim_list) != pe_dim:
            raise ValueError(
                f"Got {config.rope_dim_list} but expected positional dim {pe_dim}"
            )
        ...
        if self.text_projection == "linear":
            self.txt_in = TextProjection(
                ...
                self.hidden_size,
                get_activation("silu", return_cls=True),
            )
        elif self.text_projection == "single_refiner":
            self.txt_in = SingleTokenRefiner(
                self.text_states_dim, config.hidden_size, config.heads_num, depth=2
            )
        else:
        ...
        self.time_in = TimestepEmbedder(
            self.hidden_size, get_activation("silu", return_cls=True)
        )

        # text modulation
        self.vector_in = MLPEmbedder(self.clip_states_dim, self.hidden_size)
