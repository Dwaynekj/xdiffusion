...
from xdiffusion.layers.norm import DynamicTanhNorm
...
class DyTBlock(torch.nn.Module):
    """A PixArt block with adaptive layer norm (adaLN-single) conditioning.

    Uses Dynamic Tanh instead of layer norm.
    """
    ...
    def __init__(
        ...
        **block_kwargs,
        ...
        self.norm1 = DynamicTanhNorm(hidden_size)
        ...
        self.norm2 = DynamicTanhNorm(hidden_size)
...
class DyTFinalLayer(torch.nn.Module):
    ...
    def __init__(self, hidden_size, patch_size, out_channels):
        super().__init__()
        self.norm_final = DynamicTanhNorm(hidden_size)
...
class DyTScoreNetwork(torch.nn.Module):
    """
        ...
        self.blocks = torch.nn.ModuleList(
            [
                DyTBlock(
                    ...
                    norm_layer=DynamicTanhNorm,
                )
                for i in range(depth)
            ]
        )
        self.final_layer = DyTFinalLayer(hidden_size, patch_size, self.out_channels)
